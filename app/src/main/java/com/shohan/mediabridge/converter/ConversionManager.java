...
public class ConversionManager {
    ...
    public enum VideoFormat {
        ...
        FMT_3GP_352("3GP 352x288 - Feature Phone","3gp",352,288,"h263");
        public final String label,ext,codec; public final int w,h;
    ...
    public static void convertVideo(Context ctx,int taskId,String in,String outDir,VideoFormat fmt,Callback cb){
        String base = (in!=null&&!in.isEmpty()) ? FileUtils.base(new File(in).getName()) : String.valueOf(taskId);
        String out  = outDir+File.separator+base+"_conv."+fmt.ext;
        long totalMs=0;
        try{MediaMetadataRetriever mmr=new MediaMetadataRetriever();mmr.setDataSource(in);
            String d=mmr.extractMetadata(MediaMetadataRetriever.METADATA_KEY_DURATION);
            if(d!=null)totalMs=Long.parseLong(d);mmr.release();}catch(Exception ignored){}
        final long total=totalMs;
        String cmd="-y -i \""+in+"\""+
            " -vf scale="+fmt.w+":"+fmt.h+":force_original_aspect_ratio=decrease,pad="+fmt.w+":"+fmt.h+":(ow-iw)/2:(oh-ih)/2"+
            " -vcodec "+fmt.codec+" -r 15 -b:v 128k"+
            " -acodec aac -ar 22050 -ac 1 -b:a 32k -map_metadata -1 \""+out+"\"";
        FFmpegSession session=FFmpegKit.executeAsync(cmd,
            s->{sessionMap.remove(taskId);
                ...
                else cb.onFailure("Conversion failed");},
            log->{},
    ...
    // ── AUDIO (Media3 — accepts content URI directly) ─────────────────────
    public static void convertAudio(Context ctx,int taskId,String displayName,String inputUriStr,String outDir,AudioFormat fmt,Callback cb){
        String base=FileUtils.base(displayName!=null&&!displayName.isEmpty()?displayName:String.valueOf(taskId));
        String out=outDir+File.separator+base+"_conv."+fmt.ext;
        ...
        activeTransformer=t;
        t.start(new EditedMediaItem.Builder(androidx.media3.common.MediaItem.fromUri(inputUri))
            .setRemoveVideo(true).build(),out);
    }

    // ── IMAGE (ContentResolver stream — no temp file) ─────────────────────
    public static void convertImage(Context ctx,int taskId,String displayName,String inputUriStr,String outDir,ImageFormat fmt,Callback cb){
        String base=FileUtils.base(displayName!=null&&!displayName.isEmpty()?displayName:String.valueOf(taskId));
        String out=outDir+File.separator+base+"_conv.jpg";
        new Thread(()->{
            try{
                Bitmap bmp;
                try(InputStream is=ctx.getContentResolver().openInputStream(Uri.parse(inputUriStr))){
                    if(is==null){cb.onFailure("Cannot open image");return;}
